...
- Build a fresh `Y.Doc` from the current projected Plate value, encode it, and replace the row. Take the same `pg_advisory_xact_lock(hashtextextended(materialId, 0))` that `load` and `store` already use, so compaction serializes against concurrent document I/O. Checkpoint receipts are in-memory only (no `evo:checkpoints` Y.Map), so compaction has nothing transient to clear beyond the rebuilt content root.
...
- Make the room schema a real epoch. `material_yjs_documents.room_schema` already exists but is **write-only today** — `persistence.ts` inserts a literal `1` and nothing ever reads it back, while `schema:1` is hardcoded in five places. All of them must move to the stored epoch:
  ...
  - The service-command room in [collaboration/src/server.ts](collaboration/src/server.ts) (`/internal/commands`). This is the dangerous one: a command that builds `schema:1` after a bump would `openDirectConnection` on a *different, empty* room and write into a divergent document.
...
- **Key the client `Y.Doc` on the room** (and remount via `key=` / `editorGeneration` on rejection). `NoteEditorCore` must not reuse a pre-compaction document instance when the room epoch bumps.
...
- Yjs compaction tests: state shrinks, content survives, room schema bumps, a client on the old room cannot merge stale state back in.
