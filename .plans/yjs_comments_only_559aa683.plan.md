...
- Persist with bounded debounce/max-debounce and keep failed stores in memory for retry. After a successful binary store, convert the `content` shared `Y.XmlText` with the exact Slate-Yjs version used by Plate and submit `{materialId, yjsVersion, content}` to an authenticated internal Go projection endpoint. Retry any row whose `projected_version < stored_version`; skip revision bumps when projected content is unchanged.
- Treat Plate/Hocuspocus “synced” as in-memory server convergence only. For durable `Saved`, the client sends a `checkpoint-request` **stateless** message (random receipt ID, held in memory on the sidecar—never written into the Y.Doc). After PostgreSQL commits, broadcast `checkpoint-persisted` with claimed IDs, `yjsVersion`, measured metrics, and optional `limitCode`. Track pending receipts in a Set so concurrent edits cannot orphan an earlier request. Keep `mod+s` as a flush of the same debounced path (prevents the browser save dialog; no second checkpoint implementation).
- Enforce document limits (2 MiB / 10k nodes / depth 16) on the sidecar only—`beforeHandleMessage` (amortized measurement; allow shrink-only edits once over limit) and `store` as backstop. Reject with machine-readable codes via `document-rejected`: per-connection close for inbound violations; room broadcast + evict if store fails (Hocuspocus otherwise leaves the room live and unsavable). Clients remount the editor under an `editorGeneration` key and discard the forked Y.Doc; do not rely on token invalidation alone when the room string is unchanged. Footer stats come from server metrics on `checkpoint-persisted`, not client-side walks.
...
- Add an internal service-authenticated projection endpoint/store method. It must validate the Plate envelope with `materialdoc` (same depth/size/node caps as the sidecar), row-lock the material, ignore stale Yjs versions, update `materials.content`, increment the material revision (skip when content is byte-identical), upsert the daily revision, reconcile flashcard stats, and mark the Yjs version projected.
...
- Delete the five-second REST autosave, local revision refs, `replaceEditorDocument`, draft unload warnings, suggestion status, and client-side document-limit prechecks. Replace them with connecting/synced/saved/offline/error driven by the stateless checkpoint receipt above; never label provider `_isSynced` as “Saved”. On `document-rejected`, toast and remount via `editorGeneration` so the next connect reloads the durable room.
...
- Keep Yjs garbage collection enabled and monitor encoded document growth. Treat document compaction or rebasing as a separately tested maintenance operation, not an automatic launch-time task. Compaction rebuilds from projected Plate JSON and bumps the room schema epoch; there is no `evo:checkpoints` Y.Map to clear (receipts are in-memory only).