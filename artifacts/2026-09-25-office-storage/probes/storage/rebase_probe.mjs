...
const captured = (await office.applyOfficeCommands(bytes, seed, commands(0, 10))).state;
const B = await office.exportOffice(bytes, checkpoint(format, bytes, captured), DETERMINISM('job-r'));
const latest = (await office.applyOfficeCommands(bytes, checkpoint(format, bytes, captured), commands(10, 10))).state;
