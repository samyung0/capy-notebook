...
def main():
    ...
    templates = json.loads((FIXTURES / "challenge_templates.json").read_text(encoding="utf-8"))
    review = None
    if args.review:
        review = json.loads(args.review.read_text(encoding="utf-8"))
        assert review["status"] == "approved"
        assert review["templates_sha256"] == hashlib.sha256((FIXTURES / "challenge_templates.json").read_bytes()).hexdigest()
