...
def main():
    ...
    parser.add_argument("--output", type=Path, default=FIXTURES / "corpus.jsonl")
    args = parser.parse_args()
    plan = json.loads((FIXTURES / "curriculum.json").read_text(encoding="utf-8"))
    reviews = json.loads((FIXTURES / "reviewed_seeds.json").read_text(encoding="utf-8"))
    exceptions = json.loads((FIXTURES / "label_exceptions.json").read_text(encoding="utf-8"))
    for name, expected_hash in exceptions["source_sha256"].items():
        assert hashlib.sha256((FIXTURES / "seeds" / name).read_bytes()).hexdigest() == expected_hash, f"Label audit is stale: {name}"
    out = args.output
    questions = []
    coverage = {}
    used_exceptions = set()
    for path in sorted((FIXTURES / "seeds").glob("*.json")):
        ...
        if review:
            ...
            if language != "en":
                source = FIXTURES / "seeds" / f"{domain}.en.json"
        ...
        native_path = FIXTURES / "native" / path.name
        native_rows = {}
        native_review = None
