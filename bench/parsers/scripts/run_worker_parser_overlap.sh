...
parser_name=capy-overlap-parser
release_sha=2725da1cbe39407786c9b3bdcd40fe68e0149046
parse_names=()

cleanup() {
  for name in "${parse_names[@]}" capy-overlap-warm "$parser_name"; do
...
build_fixture_set() {
  ...
  docker run --rm \
    -v "$spool:/spool" \
    -v "$bench_root/bench/parsers/scripts/build_worker_stress_fixtures.py:/build_fixtures.py:ro" \
    capy-parser-stress:current \
...
run_parse_worker() {
  ...
  docker run -d \
    ...
    --pids-limit 128 \
    -v "$spool:/var/lib/capy-parse" \
    -v "$bench_root/bench/parsers/scripts/bench_worker_job.py:/bench_worker_job.py:ro" \
    -e PARSER_URL=http://127.0.0.1:8090/file_parse \
    -e PARSER_TOKEN=worker-stress-token \
    -e RELEASE_SHA="$release_sha" \
    -e CAPY_PARSE_METHOD=auto \
    -e CAPY_PARSE_SHARED_DIR=/var/lib/capy-parse \
    -e PARSER_TIMEOUT=2400 \
    -e CAPY_PARSE_SLICE_TIMEOUT=600 \
    -e CAPY_CAPTION_CONCURRENCY=4 \
...
run_parse_worker capy-overlap-warm \
  "sources/overlap-warm-${tag}.pdf" "overlap-warm-${tag}"
docker wait capy-overlap-warm >/dev/null
docker logs capy-overlap-warm
docker rm capy-overlap-warm >/dev/null
...
for worker_number in 1 2 3 4; do
  ...
  name="capy-overlap-parse-${worker_number}"
  parse_names+=("$name")
...
bash "$bench_root/bench/parsers/scripts/run_worker_memory_concurrency.sh" \
  8 120 1g 1g 1.0 60
