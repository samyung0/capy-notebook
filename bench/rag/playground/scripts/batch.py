...
from common import CONFIGS, LOCAL, REPO  # noqa: E402
...
def turn(server: str, config: dict, question: str) -> dict:
    req = urllib.request.Request(
        f"{server}/api/turn",
        data=json.dumps({"config": config, "question": question, "history": []}).encode(),
    ...
    with urllib.request.urlopen(req, timeout=1800) as response:
        for line in response:
            ...
            if kind == "tool_start":
                ...
                result["citations"] = [(c["fileName"], c.get("pageStart")) for c in event["citations"]]
            elif kind == "done":
                ...
                result["run_id"], result["elapsed_seconds"] = event["id"], event["elapsed_seconds"]
    return result
...
def main() -> None:
    ...
    workspaces = json.loads((REPO / "bench/rag/fixtures/local/2026-09-09-odl-agentic/workspaces.json").read_text())["arms"]
    workspace = next((w for w in workspaces.values() if w["workspace_id"] == config.get("workspace_id")), None)
    wanted = set(args.ids.split(",")) if args.ids else None
    selected = [q for q in questions() if (wanted and q["id"] in wanted) or (args.split and q.get("split") == args.split)]
    if not selected:
    ...
    for q in selected:
        ...
        rows.append({"id": q["id"], "question": q["question"], "required_claims": q.get("required_claims"), **result})
        print(f"   run={result.get('run_id')} {result.get('elapsed_seconds')}s calls={result.get('calls')} error={result.get('error')}")
        print("   answer:", result["answer"][:500].replace("\n", " "), flush=True)
        print("   claims:", q.get("required_claims"), flush=True)
        out.write_text(json.dumps({"config": args.config, "rows": rows}, ensure_ascii=False, indent=1))
    print(f"saved {out}")
