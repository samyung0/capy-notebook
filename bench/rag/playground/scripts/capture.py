...
def render(pdf: Path, page: int, bbox: list[float] | None, max_edge: int) -> tuple[bytes, list[float]]:
    ...
    with pymupdf.open(pdf) as doc:
        ...
        if bbox:
            ...
            clip = pymupdf.Rect(
                rect.x0 + rect.width * x0 / 1000, rect.y0 + rect.height * y0 / 1000,
...
async def ocr_transcribe(data_url: str, route: str) -> str:
    ...
    if not key:
        raise RuntimeError("ALIBABA_API_KEY is not set; ocr mode needs the Beijing workspace key")
...
def make_handler(config: dict[str, Any], resolver: PdfResolver, state: dict[str, Any], question: str):
    ...
    async def handler(args: dict[str, Any], ctx) -> tools.ToolResult:
        if len(state["captures"]) >= config["limits"]["captures_per_turn"]:
            return tools._refused("capture_page attempt limit for this turn is used up.", code="limit_reached")
        ...
        if cap["require_seen_page"] and not any(
            p.file_id == file_id and p.page_start and p.page_start <= page <= (p.page_end or p.page_start)
            for p in ctx.citations
        ):
            return tools._refused("Retrieve a passage that shows this page first, then capture it.")
        ...
        label = f"{scope.file_names[0]} page {page}" + (f" region {[int(v) for v in box]}" if bbox else "")
        from PIL import Image

        with Image.open(io.BytesIO(jpeg)) as im:
            width, height = im.size
        record = {
            "n": n, "call_id": args.get("_tool_call_id"), "file_id": file_id, "page": page,
            "bbox": box, "mode": cap["mode"], "image": str(image_path.relative_to(LOCAL)),
            "image_bytes": len(jpeg), "image_px": [width, height], "est_image_tokens": patch_tokens(width, height),
        }
        state["captures"].append(record)
        if cap["mode"] == "pixels":
            state["images"][args["_tool_call_id"]] = (f"capture_page result {n}: {label}", data_url)
            ...
            prompt = cap["caption_prompt"].format(question=question) if cap["question_aware"] else IMAGE_PROMPT
            text = (await models.caption_image(data_url, prompt, best_effort=False)).strip()
            note = f"Model-generated description of {label}. It may omit or misread content."
        record.update(text=text if cap["mode"] != "pixels" else "", elapsed_seconds=time.perf_counter() - started)
        if not text:
            return tools._failed("capture_page returned no usable content for that region.")
        ...
        existing = [
            i + 1 for i, p in enumerate(ctx.citations)
            if p.file_id == file_id and not p.chunk_id.startswith("capture:")
            and p.page_start and p.page_start <= page <= (p.page_end or p.page_start)
        ]
        if cap["citation"] == "page" and existing:
            ...
            return tools.ToolResult(text_parts=[note] if cap["mode"] == "pixels" else [note, text])
        passage = Passage(
            chunk_id=f"capture:{file_id}:{page}:{','.join(str(int(v)) for v in box)}:{n}",
            file_id=file_id, file_name=scope.file_names[0], chunk_idx=-1,
            section_path=f"captured page {page}", text=text, hit_text=text,
            page_start=page, page_end=page, regions=[{"page": page, "bbox": box, "space": "page-1000-topleft"}],
        )
...
def image_part(data_url: str, provider_slug: str, detail: str | None = None) -> dict[str, Any]:
    if provider_slug == "anthropic":
        header, data = data_url.split(",", 1)
        return {"type": "image", "source": {"type": "base64", "media_type": header[5:].split(";")[0], "data": data}}
...
def inject_images(
    messages: list[dict[str, Any]], images: dict[str, tuple[str, str]], provider_slug: str = "", detail: str | None = None
) -> list[dict[str, Any]]:
    ...
    for i, message in enumerate(messages):
        ...
        if pending and (following is None or following.get("role") != "tool"):
            content: list[dict[str, Any]] = [{"type": "text", "text": "Images from the capture_page calls above:"}]
...
def check() -> None:
    ...
    assert roles == [
        ...
        ("user", None), ("assistant", None), ("tool", "c"), ("user", None),
    ], roles
    assert out[5]["content"][2]["image_url"]["url"] == "data:a"
    assert out[-1]["content"][2]["image_url"]["url"] == "data:c"
    assert inject_images(msgs, {}) is msgs
    claude = inject_images(msgs, {"a": ("A", "data:image/jpeg;base64,QUJD")}, "anthropic")
    assert claude[5]["content"][2] == {"type": "image", "source": {"type": "base64", "media_type": "image/jpeg", "data": "QUJD"}}
    assert _OSS_LINK.sub("[x]", "a ![h.jpeg](http://oss/x?token=1) b") == "a [x] b"
    assert patch_tokens(1568, 1211) == 56 * 44 and patch_tokens(28, 28) == 1
    assert image_part("data:image/jpeg;base64,QQ==", "openai", "low")["image_url"]["detail"] == "low"
    assert "detail" not in image_part("data:image/jpeg;base64,QQ==", "zai", "low")["image_url"]
    import pymupdf
    ...
    try:
        ...
        assert Image.open(io.BytesIO(full)).size == (400, 200) and box == [0, 0, 1000, 1000]
        assert Image.open(io.BytesIO(crop)).size == (400, 400) and cbox == [0, 0, 500, 1000]
        for bad in ([0, 0, 0, 10], 3):
            try:
                render(path, 1, None if bad == 3 else bad, 100) if bad != 3 else render(path, 3, None, 100)
            except ValueError:
                continue
