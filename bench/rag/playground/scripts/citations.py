...
def render_structured(items: list[dict], known: int) -> tuple[str, list[int]]:
    ...
    for item in items:
        ...
        marks = "".join(f"[{r.number(n)}]" for n in item.get("passages", []) if 1 <= n <= known)
