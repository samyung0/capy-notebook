...
against the same DB and embedding provider the retrieval service uses:
    ...
    docker compose exec retrieval python bench/rag/scripts/rag_eval.py <workspace_id> \
        bench/rag/fixtures/questions.json
...
The sets under ``bench/rag/fixtures/`` are keyed to the lab corpus kept on the
ingest host in ``/opt/capy-rag-lab/samples``; re-chunking moves the indices,
...
if __name__ == "__main__":
    ws = sys.argv[1]
    default = Path(__file__).resolve().parent.parent / "fixtures" / "questions.json"
