...
**Reindex.** A workspace pins `embedding_model_key` for life. There is no job that re-embeds a corpus. Cost scales with everything they ever uploaded, not with the last file. Parked on purpose: retargeting the default only affects new workspaces; a model in use can never be deleted or lose its provider; every `embedding_dim` in use must keep at least one live model. If you ever build it, it needs progress, credit reservation, and a rewrite of vectors + `rag_contents` + workspace pins per content, while search is blocked or stale.
