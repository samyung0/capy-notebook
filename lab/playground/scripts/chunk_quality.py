...
from common import LOCAL, REPO, PdfResolver, prepare_environment
...
def score_chunk(text: str, page_texts: list[str], coverage: float | None) -> dict:
    ...
    if not text_layer:
        ...
        if r < 0.85:
            reasons.append(
                f"chunk text differs from the source text layer (match {r:.2f})"
            )
...
async def run(target: str, workspace_id: str, output: Path) -> None:
    ...
    for r in rows:
        if r["file_id"] in docs and r["page_start"]:
            for p in range(r["page_start"], (r["page_end"] or r["page_start"]) + 1):
                page_tokens.setdefault((r["file_id"], p), Counter()).update(
                    tokens(r["text"])
                )
    ...
    for r in rows:
        if r["file_id"] in missing or not r["page_start"]:
            chunks[r["id"]] = {
                "score": None,
                "reasons": [missing.get(r["file_id"], "no page model")],
                "signals": {},
            }
            continue
        span = range(r["page_start"], (r["page_end"] or r["page_start"]) + 1)
        texts = [
            docs[r["file_id"]][p - 1] for p in span if 1 <= p <= len(docs[r["file_id"]])
        ]
        covs = [
            coverage[(r["file_id"], p)] for p in span if (r["file_id"], p) in coverage
        ]
        result = score_chunk(r["text"], texts, min(covs) if covs else None)
        result.update(
            file=r["name"], chunk_idx=r["chunk_idx"], pages=[span.start, span[-1]]
        )
        chunks[r["id"]] = result
        for p in span:
            pages.setdefault(r["file_id"], {})[str(p)] = {
                "coverage": coverage.get((r["file_id"], p))
            }
        ...
        json.dumps(
            {
                ...
                "summary": {
                    "chunks": len(chunks),
                    "scored": len(scored),
                    "unscored_files": missing,
                    "median": statistics.median(scored) if scored else None,
                    "below_0.7": sum(s < 0.7 for s in scored),
                    "below_0.5": sum(s < 0.5 for s in scored),
                },
                "chunks": chunks,
                "pages": pages,
            },
    ...
    reasons = Counter(
        reason.split(" (")[0] for c in chunks.values() for reason in c["reasons"]
    )
    print(
        json.dumps(
            {
...
def check() -> None:
    ...
    assert (
        table["signals"]["table_consistency"] == 2 / 3
        and "uneven" in table["reasons"][0]
    ), table
...
def main() -> None:
    ...
    asyncio.run(
        run(
            args.target,
            args.workspace,
            LOCAL / "quality" / f"{args.target}-{args.workspace}.json",
        )
    )
