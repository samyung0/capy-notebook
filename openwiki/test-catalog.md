...
| [`src/lib/errors.test.ts`](../src/lib/errors.test.ts) | Normalizes API, network, cancellation, quota, file-cap, credits, and chunk-load failures into safe UI error kinds and actions. |
...
| [`pipeline/tests/test_chunking.py`](../pipeline/tests/test_chunking.py) | Heading breadcrumbs, page/bbox regions, table/equation/figure handling, oversized-block splitting, CJK bigram tokenizer, CJK-aware token estimate, malformed bbox coordinates skipped rather than crashing the job. |
