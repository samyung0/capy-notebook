...
SYSTEM_PROMPT = (
    ...
    "1. Browse or search the library to see what it holds for the request. "
    "browse_knowledge shows what one topic covers and in which roles; "
    "search_knowledge finds a specific role or a specific idea inside it.\n"
    ...
    "- capture_knowledge_page shows you a page of the book as it is printed. Use "
    "it when a passage header carries a low extraction-confidence note, or when "
    "the material depends on a figure, a table's layout or a formula, and read "
    "the image directly instead of guessing from the text.\n"
...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "search_knowledge": (
        ...
        "reference. `topics` takes topic ids from the catalog in the "
        "browse_knowledge description. Use this when you need a specific role or "
        "a specific idea; an empty result under a role filter reports what those "
        "topics do hold by role, so relax the filter on purpose instead of "
        "rewording."
    ),
    "browse_knowledge": (
        "List what the library holds for one catalog topic: verified excerpt "
        "counts by role and by book, then a page of excerpts with their section "
        "paths and synopses. Use it to see whether the library covers a topic at "
        "all, and which roles it can supply."
    ),
}
