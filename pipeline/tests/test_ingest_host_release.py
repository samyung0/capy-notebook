...
REMOTE_SCRIPT = REPO_ROOT / "scripts/deploy/ingest-host-remote-release.sh"
...
def _release_harness(tmp_path: Path) -> tuple[dict[str, str], Path, Path, Path]:
    ...
    secret_env = tmp_path / "ingest.env"
...
if args and args[0] == "compose":
    ...
    if command == "build":
        if os.environ.get("FAKE_FAIL_BUILD") == "1":
            save()
            raise SystemExit(1)
        state["images"][f"evo-ingest-parser:{sha}"] = sha
        state["images"][f"evo-ingest-pipeline:{sha}"] = sha
        save()
        raise SystemExit(0)
    if command == "stop":
...
""",
    ...
    env = {
        **os.environ,
        "EVO_INGEST_COMPOSE": "deploy/docker-compose.ingest-host.yml",
        "EVO_INGEST_HEALTH_ATTEMPTS": "1",
        "EVO_INGEST_HEALTH_INTERVAL": "0",
        "EVO_INGEST_PENDING_STATE": str(pending),
        "EVO_INGEST_RELEASE_ENV": str(release_env),
        "EVO_INGEST_REPO": str(repo),
        "EVO_INGEST_SECRET_ENV": str(secret_env),
...
def _assert_running_revision(state_path: Path, revision: str) -> None:
    ...
    assert services == {
        ...
        for service in ("parser", "parse-coordinator", "worker", "host-sampler")
    }
...
def test_cleanup_restores_previous_release_after_prepare(tmp_path: Path) -> None:
    ...
    assert services["parse-coordinator"]["running"] is False
