...
func TestBlobRefcountQueuesOnlyUnreferencedObjects(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, ownerID, WorkspaceCreate{Name: "Blob workspace", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestArtifactCacheRefsSurviveFileDelete(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, ownerID, WorkspaceCreate{Name: "Cache workspace", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBlobReferenceCancelsQueuedDeletion(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, ownerID, WorkspaceCreate{Name: "Requeue workspace", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExpiredBlobDeletionClaimCannotBeResurrected(t *testing.T) {
	...
	workspace, err := s.CreateWorkspace(ctx, ownerID, WorkspaceCreate{Name: "Claimed blob", Tags: nil})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloneThenDeleteKeepsTheSurvivingCopy(t *testing.T) {
	...
	source, err := s.CreateWorkspace(ctx, ownerID, WorkspaceCreate{Name: "Cloneable", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAbandonedUploadQueuesBothPaths(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, ownerID, WorkspaceCreate{Name: "Upload workspace", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
