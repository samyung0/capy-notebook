...
func TestWorkspaceFileCapRejectsThe101stFile(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, owner, WorkspaceCreate{Name: "Cap", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPendingUploadSessionsOccupyFileSlots(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, owner, WorkspaceCreate{Name: "Slots", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
	workspaceLimit := mustPlanLimits(t, s, PlanFree).FilesPerWorkspace
...
func TestExpiredUploadSessionsDoNotOccupyFileSlots(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, owner, WorkspaceCreate{Name: "Expired", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
	workspaceLimit := mustPlanLimits(t, s, PlanFree).FilesPerWorkspace
...
func TestWorkspacePayloadReportsFileLimit(t *testing.T) {
	...
	ws, err := s.CreateWorkspace(ctx, owner, WorkspaceCreate{Name: "Limit", Tags: []TagRef{}})
	if err != nil {
		t.Fatal(err)
	}
