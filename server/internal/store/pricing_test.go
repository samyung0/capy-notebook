...
func TestCreditsForTokensDifferByModel(t *testing.T) {
	flash := TokenRates{MicrosPerInputToken: 250, MicrosPerOutputToken: 1000, Model: models.Ref{ProviderSlug: "deepseek", ModelSlug: "deepseek-flash"}, ModelVersion: 1}
	expensive := TokenRates{MicrosPerInputToken: 775, MicrosPerOutputToken: 3100, Model: models.Ref{ProviderSlug: "deepseek", ModelSlug: "test-expensive-model"}, ModelVersion: 1}
	in, out := int64(1_000), int64(1_000)
	flashC := CreditsForTokens(flash, KindLLM, in, out, 0)
	expensiveC := CreditsForTokens(expensive, KindLLM, in, out, 0)
	if flashC != 1_250_000 {
		t.Fatalf("flash credits = %d", flashC)
	}
	if expensiveC <= flashC {
		t.Fatalf("expensive model (%d) must cost more than flash (%d)", expensiveC, flashC)
	}
}
