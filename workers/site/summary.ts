...
export function renderSummary(
  ...
  const files = (entries: WorkspaceSummary['files']) =>
    `<ul class="summary-files">${entries.map((entry) => `<li>${fileIcon}<span>${escapeHTML(entry.name)}</span><small>${escapeHTML(fileMeta(entry))}</small></li>`).join('')}</ul>`;
  const section = (name: string, entries: WorkspaceSummary['files']) =>
    `<section class="summary-chapter"><h2>${escapeHTML(name)}</h2>${files(entries)}</section>`;
